...
export const createCryptoCoinNetwork = async (req, res) => {
  try {
    ...
    //   data: req.body,
    // });
    const { name, kode, admin_fee, rpc_url, status, chain_id, coin_network_id } = req.body;
    const file = req.file;
    ...
    const newCoin = await saveCryptoCoinNetwork( name, kode, admin_fee, logoPath, rpc_url, status, chain_id, coin_network_id );
    res.status(201).json({
      msg: "Crypto coin network created successfully",
      data: newCoin,
    });
...
export const updateCryptoCoinNetwork = async (req, res) => {
  try{
    ...
    const {name, kode, admin_fee, rpc_url, status, chain_id, coin_id} = req.body;
